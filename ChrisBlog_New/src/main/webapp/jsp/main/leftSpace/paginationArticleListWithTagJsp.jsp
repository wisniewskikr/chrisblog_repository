...
<c:if test="${command.pagesCount > 1}">
	...
	<div class="pagination">
		...
		<c:choose>
			...
			<c:otherwise>
				<a id="first" href="tag/page/1/${command.articleTag.uniqueName}" class="first">« First</a>
			</c:otherwise>
		</c:choose>
		...
		<c:choose>
			...
			<c:otherwise>
				<a id="previous" href="tag/page/${command.pageCurrent - 1}/${command.articleTag.uniqueName}" class="previouspostslink">« Prev</a>
			</c:otherwise>
		</c:choose>			
		...
		<c:forEach begin="1" end="${command.pagesCount}" step="1" var="page">			
			<c:choose>
				...
				<c:when test="${command.pageCurrent != page && page >= min && page <= max}">
					<a id="page_${page}" href="tag/page/${page}/${command.articleTag.uniqueName}">${page}</a>
				</c:when>
			</c:choose>			
		</c:forEach>
		...
		<c:choose>
			...
			<c:otherwise>
				<a id="next" href="tag/page/${command.pageCurrent + 1}/${command.articleTag.uniqueName}" class="nextpostslink">Next »</a>
			</c:otherwise>
		</c:choose>
		...
		<c:choose>
			...
			<c:otherwise>
				<a id="last" href="tag/page/${command.pagesCount}/${command.articleTag.uniqueName}" class="last">Last »</a>
			</c:otherwise>
		</c:choose>
